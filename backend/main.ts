...
// Admin users (skip rate limits and budget)
const ADMIN_USERNAMES = (Deno.env.get("ADMIN_USERNAMES") || "rahulyal").split(",");
...
app.post("/v1/messages", async (c: Context) => {
  ...
  const isAdmin = ADMIN_USERNAMES.includes(authUser.username);

  // Check rate limit (skip enforcement for admins, but still track)
  const rateLimit = await checkRateLimit(authUser.id);
  if (!isAdmin && !rateLimit.allowed) {
    return c.json(
      {
  ...
  // Check monthly budget (skip for admins)
  if (!isAdmin) {
...
app.post("/admin/reset-budget/:username", async (c: Context) => {
  const authUser = await getAuthUser(c);
  if (!authUser) return c.json({ error: "Unauthorized" }, 401);

  if (!ADMIN_USERNAMES.includes(authUser.username)) {
  ...
  for await (const entry of iter) {
    if (entry.value.username === targetUsername) {
      targetUser = entry.value;
      break;
    }
  }

  if (!targetUser) {
    return c.json({ error: `User "${targetUsername}" not found` }, 404);
  }
  ...
  return c.json({
    success: true,
    message: `Budget reset for ${targetUsername} (${monthKey})`,
  });
});

// List all users with usage (admin only)
app.get("/admin/users", async (c: Context) => {
  const authUser = await getAuthUser(c);
  if (!authUser) return c.json({ error: "Unauthorized" }, 401);
  ...
  users.sort((a, b) => b.usage.cost - a.usage.cost);

  return c.json({
    month: monthKey,
    totalUsers: users.length,
    totalCost: parseFloat(users.reduce((sum, u) => sum + u.usage.cost, 0).toFixed(4)),
